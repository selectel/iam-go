...
// Service is used to communicate with the OIDC Federations API.
type Service struct {
...
// List returns a list of OIDC Federations for the account.
...
// Get returns an info of OIDC Federation with federationID.
func (s *Service) Get(ctx context.Context, federationID string) (*GetResponse, error) {
	...
	path, err := url.JoinPath(apiVersion, "federations", "oidc", federationID)
	if err != nil {
		return nil, iamerrors.Error{Err: iamerrors.ErrInternalAppError, Desc: err.Error()}
	}
...
// Update updates existing OIDC Federation.
...
// Delete deletes an OIDC Federation from the account.
